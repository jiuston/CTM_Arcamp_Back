...
public class JugadorController {
    ...
    @POST
    @Path("search")
    public CustomPage<JugadorOutDto> search(@RequestBody List<Filter> filters, @QueryParam("page") @DefaultValue("0") int page, @QueryParam("size") @DefaultValue("10") int size, @QueryParam("sortField") @DefaultValue("id") String sortField, @QueryParam("sortDirection") @DefaultValue("ASCENDING") SortDirection sortDirection) {
        CustomPage<JugadorOutDomain> jugadores = jugadorPort.search(new Search(filters, sortField, sortDirection, page, size));
        return new CustomPage<>(jugadores.getContent().stream().map(mapper::toOutDto).toList(), page, size, jugadores.getTotalElements(), jugadores.getTotalPages());
    }
}
